...
class Debug
{
    ...
    public static $config = array(
        ...
        'file_log' => 'notice.txt',
        /**
         * @desc Файл для записи ошибок вызванных try catch сообщений.
         * @var string
         */
        'file_user' => 'user.txt',
        /**
        ...
        if ($errno == E_ERROR || $errno == E_USER_ERROR) {
            ...
            if (is_file($filename)) {
                ...
                foreach ($lines as $line) {
                    list($time, $date, $file, $line, $message) = explode('|', $line);
        ...
        try {
            if ($errno == E_ERROR || $errno == E_USER_ERROR) {
                if (self::$config ['die_on_error']) {
                    ...
                    throw new ErrorException($log_text);
                }
            } else {
                throw new ErrorException($log_text);
            }
        } catch (Exception $e) {
            self::outputErrors($e, $errcontext);
        }
        return true;
    }

    public static function outputErrors(Exception $exception, $errcontext = array())
    {
        ...
        echo '<i style="color: grey;">url: ' . (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '') . "</i>\n";
        if (isset($_SERVER['HTTP_REFERER'])) {
            echo '<i style="color: grey;">referer: ' . (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['REQUEST_URI'] : '') . "</i>\n";
        }
...
//            fb($errcontext, 'INFO');
     ...
     */
    public static function setOutput($database = null, $echo = true, $firebug = null, $file = null)
    {
        // БД
        if ($database === null) {
